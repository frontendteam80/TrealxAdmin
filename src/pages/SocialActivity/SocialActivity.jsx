...
//   // Helper to identify IDs from varying keys
//   function getId(obj, keys) {
//     for (const key of keys) if (obj[key] !== undefined) return obj[key];
//     return null;
//   }
...
//       const currentProjectUnits = allUnits.filter(
//         (item) =>
//           String(getId(item, ["ProjectID", "ProjectId", "projectId", "projectid"])) === String(projectId)
//       );
//       if (currentProjectUnits.length === 0) return;

//       const specificUnit = currentProjectUnits.find(
//         (unit) =>
//           String(getId(unit, ["PropertyID", "PropertyId", "propertyId", "propertyid"])) === String(propertyId)
//       );
//       if (!specificUnit) return;
...
//   // Filter displayData by current filters
//   const filterByColumns = (data) => {
//     if (!Object.keys(filters).length) return data;
//     return data.filter((item) =>
//       Object.entries(filters).every(([key, values]) => {
...
//   // Columns config per tab (using header filters)
//   const columnsConfig = {
//     Tours: [
